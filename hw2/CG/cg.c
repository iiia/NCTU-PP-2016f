...
#include <omp.h>
/* Modify lines:
 * 	before set starting vetor
 *  in conj_grad
 * 	in sprnvc()  temp comment
 *  veset
 */
...
#pragma omp parallel for
    ...
    for (j = 0; j < lastcol - firstcol + 1; j++) {
      norm_temp1 = norm_temp1 + tempx[j];
      norm_temp2 = norm_temp2 + tempz[j];
    }
...
//#pragma omp parallel shared( iv, v) private( ii, i)
//#pragma omp for
//  for ( nzv = 0 ; nzv < nz ; nzv++ ){
  while (nzv < nz) {
